...
train_ds = MOSEI(
    data_fp="/home/jmg/code/python/MML_Suite/DATA/mosei/aligned.pkl",
    split="train",
...
stats = {
    ...
    Modality.TEXT: {"sum": 0.0, "sum2": 0.0, "n": 0},
}

with torch.no_grad():
    for batch in loader:
        for mod in [Modality.AUDIO, Modality.VIDEO, Modality.TEXT]:
            if mod in batch:
                x = batch[mod]                   # e.g. [B, ...]
